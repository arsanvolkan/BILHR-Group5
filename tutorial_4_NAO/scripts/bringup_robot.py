...
class Central:
    ...
    def move_to_blob(self):
        """This function is called via button or via image_cb function and calls the predict_joints service with the
        last stored coordinates of a red blob. the return of the service should be two angles in radians
        for each joint one. At last set each joint to the returned value"""
        x = self.blob_pos_x
        y = self.blob_pos_y
        ...
        if x == 0 and y == 0:
            ...
            try:
                ...
                self.set_joint_angles(["LShoulderPitch","LShoulderRoll"],[resp.angle1-0.3, resp.angle2])
